...
public class MainMemoryWrapper implements Mic1Constants {
  ...
  public MainMemoryWrapper(Bus byte_address_bus, Bus word_address_bus ,
  ...
  public void poke() {
	...
	if (Mic1Wrapper.key_buffer.size() == 0) {
	  if (Mic1Wrapper.debug)
	    DebugFrame.text.appendText("Key not pressed\n");
	  data = 0;
        }
	else {
	  data = ((Character)Mic1Wrapper.key_buffer.elementAt(0)).charValue();
	  Mic1Wrapper.key_buffer.removeElementAt(0);
	  if (Mic1Wrapper.debug)
